#include "BinaryUtils.hpp"

namespace BinaryUtils {

// Pre-computed reflection table for 8-bit values
const uint8_t reflect_table[256] = {
    
};

uint8_t calculate_checksum8(const std::vector<uint8_t>& data) {
    uint32_t sum = 0;
    for (uint8_t byte : data) {
        sum += byte;
    }
    return static_cast<uint8_t>(256 - (sum & 0xFF));
}

uint16_t calculate_crc16(const std::vector<uint8_t>& data, 
                        uint16_t polynomial, uint16_t initial) {
    uint16_t crc = initial;
    
    for (uint8_t byte : data) {
        crc ^= static_cast<uint16_t>(byte) << 8;
        
        for (int i = 0; i < 8; ++i) {
            if (crc & 0x8000) {
                crc = (crc << 1) ^ polynomial;
            } else {
                crc <<= 1;
            }
        }
    }
    
    return crc;
}

uint32_t calculate_crc32(const std::vector<uint8_t>& data,
                        uint32_t polynomial, uint32_t initial) {
    uint32_t crc = initial;
    
    for (uint8_t byte : data) {
        crc ^= static_cast<uint32_t>(byte) << 24;
        
        for (int i = 0; i < 8; ++i) {
            if (crc & 0x80000000) {
                crc = (crc << 1) ^ polynomial;
            } else {
                crc <<= 1;
            }
        }
    }
    
    return crc ^ 0xFFFFFFFF;
}

uint8_t reflect_byte(uint8_t value) {
    return reflect_table[value];
}

uint16_t reflect_word(uint16_t value) {
    return (static_cast<uint16_t>(reflect_table[value & 0xFF]) << 8) |
           reflect_table[(value >> 8) & 0xFF];
}

uint32_t reflect_dword(uint32_t value) {
    return (static_cast<uint32_t>(reflect_table[value & 0xFF]) << 24) |
           (static_cast<uint32_t>(reflect_table[(value >> 8) & 0xFF]) << 16) |
           (static_cast<uint32_t>(reflect_table[(value >> 16) & 0xFF]) << 8) |
           reflect_table[(value >> 24) & 0xFF];
}

} // namespace BinaryUtils